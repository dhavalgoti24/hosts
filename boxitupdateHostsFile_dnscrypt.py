...
def main():
    ...
    remove_old_hosts_file(
        path_join_robust(settings["outputpath"], "hosts_boxit_dnscrypt"), settings["backup"]
    )
    if settings["compress"]:
        final_file = open(path_join_robust(settings["outputpath"], "hosts_boxit_dnscrypt"), "w+b")
...
def prompt_for_update(freshen, update_auto):
    ...
    hosts_file = path_join_robust(BASEDIR_PATH, "hosts_boxit_dnscrypt")
...
def write_opening_header(final_file, **header_params):
    ...
    preamble = path_join_robust(BASEDIR_PATH, "myhosts_dnscrypt")
